...
void setup() {
  ...
  timeButtonColor    = color( 15, 175, 205);
  addNoteButtonColor = color( 15, 175, 205);
  synthButtonColor   = color( 15, 175, 205);
  goButtonColor      = color( 20, 215,  50);
  stopButtonColor    = color(215,  20,  20);

  keyButtonHighlight     = color( 10,  80,  95);
  timeButtonHighlight    = color( 10,  80,  95);
	addNoteButtonHighlight = color( 10,  80,  95);
  synthButtonHighlight   = color( 10,  80,  95);
  goButtonHighlight      = color( 15, 115,  20);
  stopButtonHighlight    = color(180,  40,  25);

  currentColor = baseColor = color(255);

  // Positioning buttons
  keyButtonX     = 100; keyButtonY     = 100; timeButtonX  = 300; timeButtonY  = 100;
  addNoteButtonX = 100; addNoteButtonY = 300; synthButtonX = 300; synthButtonY = 300; 
  goButtonX      = 100; goButtonY      = 500; stopButtonX  = 300; stopButtonY  = 500;
...
void draw() {
  ...
  text("Key is " + frequencyToNoteMap.get(currentRootNote) + 
      (currentKeyIsMajor ? " Major: " : " Minor: ") + Arrays.toString(currentKeyNotes) +
  ...
  squareButtonMaker(keyButtonX, keyButtonY, "KEY");
  ...
  squareButtonMaker(timeButtonX, timeButtonY, "TIME");
  ...
  squareButtonMaker(addNoteButtonX, addNoteButtonY, "ADD NOTE");
  ...
  squareButtonMaker(goButtonX, goButtonY, "GENERATE!");
...
void update(int x, int y) {
  if (overKeyButton(keyButtonX, keyButtonY, squareButtonSize, squareButtonSize)) {
    keyButtonOver = true;
    timeButtonOver = addNoteButtonOver = synthButtonOver = goButtonOver = stopButtonOver = false;
  } else if (overTimeButton(timeButtonX, timeButtonY, squareButtonSize, squareButtonSize)) {
    timeButtonOver = true;
    keyButtonOver = addNoteButtonOver = synthButtonOver = goButtonOver = stopButtonOver = false;
  } else if (overAddNoteButton(addNoteButtonX, addNoteButtonY, squareButtonSize, squareButtonSize)) {
  ...
  } else if (overGoButton(goButtonX, goButtonY, squareButtonSize, squareButtonSize)) {
    goButtonOver = true;
